...
export default function WordsOfHopeScreen({
  ...
  const processCollision = (item) => {
    ...
    if (item.isCorrect) {
      ...
      if (scoreRef.current % 4 === 0) {
        ...
        safeAudio('playInvestigate');
      }

      safeAudio('playDing');
      safeAudio('playCoachTip');
  ...
  const checkFinalOutcome = () => {
    ...
    setTimeout(() => {
      setGameState(scoreRef.current >= 4 ? "RESULTS" : "GAME_OVER");
      safeAudio('stopMusic');
    }, 1000);
  };
      ...
      {gameState === "TUTORIAL" && (
        <div className="relative z-10 max-w-2xl w-full p-6 md:p-8 text-center animate-fade-in flex flex-col items-center stickman-rescue-tutorial">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-8 md:mb-12 w-full text-left max-h-[40vh] md:max-h-none overflow-y-auto md:overflow-visible pr-2 custom-scrollbar">
            {[
            ...
            ].map(({ num, badgeBg, labelColor, label, text }) => (
              <div
                key={num}
              ...
              >
                <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
                  <div
                    className={`w-6 h-6 md:w-8 md:h-8 ${badgeBg} rounded-lg flex items-center justify-center font-black text-white text-xs md:text-base`}
                  >
                    {num}
                  </div>
                  <span
                    className={`${labelColor} font-bold uppercase tracking-widest text-[8px] md:text-[10px]`}
                  >
                    {label}
                  </span>
                </div>
