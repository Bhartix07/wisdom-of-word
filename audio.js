...
class SoundEngine {
    ...
    playPop() {
        if (!this.initialized || !this.ctx || this.globalPaused) return;
    ...
    async playMenuMusic() {
        if (!this.initialized || !this.ctx || this.currentTrack === 'menu') return;
    ...
    updateMusic(trust) {
        if (!this.initialized || !this.ctx || !this.musicNodes) return;
            ...
            this.musicNodes.forEach(n => {
                try {
                    if (n.gain && n.gain.gain) {
                        n.gain.gain.linearRampToValueAtTime(0.001, this.ctx.currentTime + 0.5);
                    }
